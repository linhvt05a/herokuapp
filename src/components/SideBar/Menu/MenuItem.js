...
class MenuItem extends Component {
    ...
    handleClick = (e) => {
        if( this.props.data.href ){
            window.location = this.props.data.href;
        }
    ...
    render() {
        ...
        return (
            <li className={ data.active == true ? 'sidebar-item active' : 'sidebar-item' } onClick={this.handleClick}>
                <a className={clsname} >
                    <i className={data.icon ? data.icon : "las la-home"}></i>
                    <span className="hide-menu"><Trans>{data.label}</Trans></span>
                </a>
